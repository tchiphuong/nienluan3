                                ...
                                <div class="accordion" id="accordionExample">
                                    @foreach($brand as $key => $value)
                                        <div class="card">
                                            <div class="card-text">
                                                <b>
                                                    @if($value->brand_id==$name->brand_id)
                                                        <a class="text-danger" href="{{ URL::to('/thuong-hieu/'.$value->brand_id)}}"><i class="fa fa-angle-right"></i> {{$value->brand_name}}</a>
                                                    @else
                        ...
                        @endforeach
                            <div class="col-lg-12 text-center">
                                <div class="pagination__option">
                                    <?php
                                    $next = $brand_by_id->currentPage() + 1;
                                    $prev = $brand_by_id->currentPage() - 1;
                                    ?>
                                    @if ($brand_by_id->currentPage()>1)
                                        <a href="{{$name->brand_id}}?page=1"><i class="fa fa-angle-double-left"></i></a>
                                        <a href="{{$name->brand_id}}?page={{$prev}}"><i class="fa fa-angle-left"></i></a>
                                        @endif
                                    @for ($i = 1; $i <= $brand_by_id->lastPage(); $i++)
                                        @if ($i==$brand_by_id->currentPage())
                                            <span class="active">{{$i}}</span>
                                        @else
                                            <a href="{{$name->brand_id}}?page={{$i}}"><span>{{$i}}</span></a>
                                        @endif
                                        @endfor
                                    @if ($brand_by_id->currentPage()!=$brand_by_id->lastPage())
                                        <a href="{{$name->brand_id}}?page={{$next}}"><i class="fa fa-angle-right"></i></a>
                                        <a href="{{$name->brand_id}}?page={{$brand_by_id->lastPage()}}"><i class="fa fa-angle-double-right"></i></a>
                                    @endif
                                </div>
                            </div>
                    </div>
